    ...
    public class AluguelAction
    {
        ...
        public Aluguel Detalhe(int? idAluguel)
        {
            try
            {
                ...
                _con.Comando.CommandText = " SELECT" +
                                           " select id_cliente, nome_cliente, id_veiculo, nome_veiculo, id_aluguel, preco_aluguel, data_aluguel, data_devolucao" +
                                           " FROM vw_aluguel" +
                                           " WHERE id_aluguel = @id_aluguel";
                _con.Comando.Parameters.Add(Funcoes.ParametrosInsercao(idAluguel, "@id_cliente", DbType.Int32));
                ...
                if (dt.Rows.Count > 0)
                {
                    for(int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        aluguel.Id = Convert.ToInt32(dt.Rows[0]["id_aluguel"].ToString());
                        ...
                        aluguel.Preco = dt.Rows[0]["preco_aluguel"].ToString();
        ...
        public List<Aluguel> Lista(string nome)
        {
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    {
                        var aluguel = new Aluguel();
                        aluguel.Id = Convert.ToInt32(dt.Rows[i]["id_aluguel"].ToString());
                        ...
                        aluguel.Preco = dt.Rows[i]["preco_aluguel"].ToString();
        ...
        public bool Salvar (Aluguel aluguel)
        {
            try
            {
                ...
                if (aluguel.IdCliente == 0)
                {
                    _con.Comando.CommandText = "INSERT INTO aluguel (id_aluguel, preco_aluguel, data_aluguel, data_devolucao, estado_aluguel, fk_id_cliente, fk_id_veiculo" +
                                               " VALUES (@id_aluguel, @preco_aluguel, @data_aluguel, @data_devolucao, @estado_aluguel, @id_cliente, @id_veiculo)";
                }
                else
                {
                    _con.Comando.CommandText = "UPDATE aluguel SET id_aluguel=@id_aluguel, preco_aluguel=@preco_aluguel, data_aluguel=@data_aluguel, data_devolucao=@data_devolucao, estado_aluguel = @estado_aluguel, id_cliente = @id_cliente, id_veiculo = @id_veiculo" +
                                               "WHERE id_cliente = @id_cliente";
                    _con.Comando.Parameters.Add(Funcoes.ParametrosInsercao(aluguel.Id, "@id_aluguel", DbType.Int32));
                }
                _con.Comando.Parameters.Add(Funcoes.ParametrosInsercao(aluguel.Preco, "@preco_aluguel", DbType.String));
